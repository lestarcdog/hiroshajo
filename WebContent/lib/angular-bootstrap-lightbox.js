 ...
 */
angular.module('bootstrapLightbox').service(
		'ImageLoader',
		[
				'$q',
				function($q) {
...
angular.module('bootstrapLightbox').provider(
		...
		function() {
			...
			/**
			 * Whether images should be scaled to the maximum possible
			 * dimensions.
			 * 
			 * @type {Boolean}
			 * @name fullScreenMode
			 ...
			 * 
			 * @param {Object}
			 *            dimensions Contains the properties `windowWidth`,
			 *            `windowHeight`, `imageWidth`, and `imageHeight`.
			 * @return {Object} May optionally contain the properties
			...
			this.calculateImageDimensionLimits = function(dimensions) {
				if (dimensions.windowWidth >= 768) {
					return {
						...
						// with the goal of 30px side margins; however, the
						// actual side margins
						// will be slightly less (at 22.5px) due to the vertical
						// scrollbar
			...
			/**
			 * Calculate the width and height of the modal. This method gets
			 * called after the width and height of the image, as displayed
			 * inside the modal, are calculated.
			 * 
			 * @param {Object}
			 *            dimensions Contains the properties `windowWidth`,
			 *            `windowHeight`, `imageDisplayWidth`, and
			 *            `imageDisplayHeight`.
			 * @return {Object} Must contain the properties `width` and
			 *         `height`.
			 * @type {Function}
			...
			this.calculateModalDimensions = function(dimensions) {
				...
				// first case: the modal width cannot be larger than the window
				// width
			 ...
			 * @param {*}
			 *            image An element in the array of images.
			 * @return {Boolean} Whether the provided element is a video that is
			 *         to be embedded with an external service like YouTube. By
			 *         default, this is determined by the url not ending in
			 *         `.mp4`, `.ogg`, or `.webm`.
			 * @type {Function}
			 * @name isSharedVideo
			 * @memberOf bootstrapLightbox.Lightbox
			...
			this.$get = [ '$document', '$injector', '$uibModal', '$timeout', 'ImageLoader',
					function($document, $injector, $uibModal, $timeout, ImageLoader) {
						// optional dependency
						var cfpLoadingBar = $injector.has('cfpLoadingBar') ? $injector.get('cfpLoadingBar') : null;

						var Lightbox = {};

						/**
						 * Array of all images to be shown in the lightbox (not
						 * `Image` objects).
						 * 
						 * @type {Array}
						 * @name images
						...
						/**
						 * The index in the `Lightbox.images` aray of the image
						 * that is currently shown in the lightbox.
						 * 
						 * @type {Number}
						 * @name index
						 * @memberOf bootstrapLightbox.Lightbox
						...
						// set the configurable properties and methods, the
						...
						/**
						 * Whether keyboard navigation is currently enabled for
						 * navigating through images in the lightbox.
						 * 
						 * @type {Boolean}
						...
						/**
						 * The URL of the current image. This is a property of
						 * the service rather than of `Lightbox.image` because
						 * `Lightbox.image` need not be an object, and besides
						 * it would be poor practice to alter the given objects.
						 * 
						 * @type {String}
						 * @name imageUrl
						 * @memberOf bootstrapLightbox.Lightbox
						 ...
						 * 
						 * @param {Array}
						 *            newImages An array of images. Each image
						 *            may be of any type.
						 * @param {Number}
						 *            newIndex The index in `newImages` to set
						 *            as the current image.
						 * @param {Object}
						 *            modalParams Custom params for the angular
						 *            UI bootstrap modal (in $uibModal.open()).
						 * @return {Object} The created UI Bootstrap modal
						 *         instance.
						 * @type {Function}
						 * @name openModal
						 * @memberOf bootstrapLightbox.Lightbox
						...
						Lightbox.openModal = function(newImages, newIndex, modalParams) {
							...
							// store the modal instance so we can close it
							// manually if we need to
							Lightbox.modalInstance = $uibModal.open(angular.extend({
								'templateUrl' : Lightbox.templateUrl,
								'controller' : [ '$scope', function($scope) {
									// $scope is the modal scope, a child of
									// $rootScope
							...
							Lightbox.modalInstance.result['finally'](function() {
								// prevent the lightbox from flickering from the
								// old image when it gets
								// opened again
								Lightbox.images = [];
								Lightbox.index = 1;
								Lightbox.image = {};
						 ...
						 * 
						 * @param {*}
						 *            result This argument can be useful if the
						 *            modal promise gets handler(s) attached to
						 *            it.
						 * @type {Function}
						 * @name closeModal
						 * @memberOf bootstrapLightbox.Lightbox
						...
						/**
						 * This method can be used in all methods which
						 * navigate/change the current image.
						 * 
						 * @param {Number}
						 *            newIndex The index in the array of images
						 *            to set as the new current image.
						 * @type {Function}
						 * @name setImage
						 * @memberOf bootstrapLightbox.Lightbox
						 */
						Lightbox.setImage = function(newIndex) {
							...
							if (!Lightbox.isVideo(image)) {
								// load the image before setting it, so
								// everything in the view is
								// updated at the same time; otherwise, the
								// previous image remains while
								...
								ImageLoader.load(imageUrl).then(function() {
									...
									success({
										...
										// use the caption to show the user an
										// error
						...
						/**
						 * Call this method to set both the array of images and
						 * the current image (based on the current index). A use
						 * case is when the image collection gets changed
						 * dynamically in some way while the lightbox is still
						 * open.
						 * 
						 * @param {Array}
						 *            newImages The new array of images.
						 * @type {Function}
						 * @name setImages
						 * @memberOf bootstrapLightbox.Lightbox
						 */
						...
						// Bind the left and right arrow keys for image
						// navigation. This event
						// handler never gets unbinded. Disable this using the
						// `keyboardNavEnabled`
						// flag. It is automatically disabled when the target is
						// an input and or a
						...
						$document.bind('keydown', function(event) {
							...
							if (method !== null && [ 'input', 'textarea' ].indexOf(event.target.tagName.toLowerCase()) === -1) {
								// the view doesn't update without a manual
								// digest
								$timeout(function() {
									Lightbox[method]();
								});
...
angular.module('bootstrapLightbox').directive('lightboxSrc',
		[ '$window', 'ImageLoader', 'Lightbox', function($window, ImageLoader, Lightbox) {
			...
			var calculateImageDisplayDimensions = function(dimensions, fullScreenMode) {
				...
				if (!fullScreenMode) {
					...
					if (w < minW && h < minH) {
						// the image is both too thin and short, so compare the
						// aspect ratios to
					...
					if (w > maxW && h > maxH) {
						// the image is both too tall and wide, so compare the
						// aspect ratios
				...
				return {
					'width' : displayW || 0,
					'height' : displayH || 0
				// NaN is possible when dimensions.width is 0
				};
			};

			// format the given dimension for passing into the `css()` method of
			// `jqLite`
			...
			return {
				'link' : function(scope, element, attrs) {
					...
					var resize = function() {
						...
						var imageDisplayDimensions = calculateImageDisplayDimensions(angular.extend({
							'width' : imageWidth,
							'height' : imageHeight,
							'minWidth' : 1,
							'minHeight' : 1,
							'maxWidth' : 3000,
							'maxHeight' : 3000,
						}, imageDimensionLimits), Lightbox.fullScreenMode);
						...
						// setting the height on .modal-dialog does not expand
						// the div with the
						...
						// .modal-content has no width specified; if we set the
						// width on
						// .modal-content and not on .modal-dialog,
						// .modal-dialog retains its
						// default width of 600px and that places .modal-content
						// off center
					...
					// load the new image and/or resize the video whenever the
					// attr changes
					scope.$watch(function() {
						...
						if (!Lightbox.isVideo(Lightbox.image)) { // image
							...
							// the src attribute applies to `<video>` and not
							// `<embed-video>`
							element[0].src = src;
						}
					});

					// resize the image and modal whenever the window gets
					// resized
